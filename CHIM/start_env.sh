...
then
	echo -ne  "Starting Minime-T5 service "
	su dwemer -c /home/dwemer/minime-t5/start.sh
	check_port 8082
else
	echo "Skipping Minime-T5 service (not enabled)"
	L_MINIME=1
fi
...
if [ -z "$L_MINIME" ]; then
	echo Minime-T5 API: http://$ipaddress:8082
fi
...
echo "You can now close this window. Use Tools/Force Stop Distro.bat to fully shutdown DwemerDistro"
