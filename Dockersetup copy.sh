...
# ===================================================================
# START OF EMBEDDED start_env SCRIPT
# ===================================================================
#!/bin/bash

        /usr/local/bin/print_logo

        # Clean tmp directory - remove files older than 7 days
        find /tmp -type f -mtime +7 -delete 
        find /tmp/ -type d -mtime +7 -exec \rm -fr {} \; &>/dev/null

        # Function to check if a service port is listening
        check_port() {
            local port=\$1
            local retries=30
            local wait_time=2
        ...
        # Start Apache/PHP and PostgreSQL servers
        ...
        # Get the container's IP address for display
        ipaddress=\$(/usr/local/bin/get_ip)

        # Display connection information and running services
        cat << EOF
=======================================
...
# ===================================================================
# END OF EMBEDDED start_env SCRIPT
# ===================================================================
SCRIPT_EOF
