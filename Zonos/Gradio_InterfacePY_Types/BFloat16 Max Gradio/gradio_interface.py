...
def make_cond_dict_with_dtype(dtype=torch.bfloat16, **kwargs):
    """Wrapper that ensures consistent dtype while preserving integer indices"""
    cond_dict = make_cond_dict(**kwargs)
    
    # These should remain as integers for embedding layers
    integer_keys = {"language_id", "speaker_noised"}
    
    # Convert only floating point conditioning to model dtype
    for k, v in cond_dict.items():
        if isinstance(v, torch.Tensor) and k not in integer_keys:
            cond_dict[k] = v.to(dtype=dtype)
        elif isinstance(v, torch.Tensor) and k in integer_keys:
            # Keep as Long for embedding layers
            cond_dict[k] = v.to(dtype=torch.long)
...
def load_model_if_needed(model_choice: str):
    ...
    if CURRENT_MODEL_TYPE != model_choice:
        ...
        print(f"Loading {model_choice} model with PROPER BFloat16 optimizations...")
        
        with torch.no_grad():
            CURRENT_MODEL = Zonos.from_pretrained(model_choice, device=device)
            # Model already loads in BFloat16 - work with it!
            
            # Just ensure everything is consistent BFloat16
            print("Ensuring consistent BFloat16 across all components...")
            CURRENT_MODEL = CURRENT_MODEL.to(torch.bfloat16)
        
        CURRENT_MODEL.requires_grad_(False).eval()
        CURRENT_MODEL_TYPE = model_choice
        print(f"{model_choice} model loaded with consistent BFloat16!")
...
def generate_audio(
    ...
    """Generate audio with proper BFloat16 optimizations."""
    ...
    # Get the model's actual dtype (should be bfloat16)
    model_dtype = next(selected_model.parameters()).dtype
    print(f"Model dtype: {model_dtype}")

    # Speaker embedding - will automatically be bfloat16
    if speaker_audio is not None and "speaker" not in unconditional_keys:
        if speaker_audio != SPEAKER_AUDIO_PATH:
            print(f"Computing speaker embedding in {model_dtype}")
            wav, sr = torchaudio.load(speaker_audio)
            wav = wav.to(device)
            SPEAKER_EMBEDDING = selected_model.make_speaker_embedding(wav, sr)
            # Don't convert - it's already the right dtype
            SPEAKER_AUDIO_PATH = speaker_audio

    # Prefix audio - autoencoder handles dtype internally
    ...
    # Create tensors in the SAME dtype as model (for floating point values)
    emotion_tensor = torch.tensor(
        list(map(float, [e1, e2, e3, e4, e5, e6, e7, e8])), 
        device=device, 
        dtype=model_dtype
    )

    vq_val = float(vq_single)
    vq_tensor = torch.tensor(
        [vq_val] * 8, 
        device=device, 
        dtype=model_dtype
    ).unsqueeze(0)

    # Use dtype-aware conditioning
    cond_dict = make_cond_dict_with_dtype(
        text=text,
        language=language,
    ...
    # Prepare conditioning - all dtypes should match now
    ...
    # Decode - autoencoder handles dtype internally, outputs float32
        ...
        with gr.Row():
            with gr.Column():
                model_choice = gr.Dropdown(
                    ...
                    label="Zonos Model Type (BFloat16 Optimized)",
