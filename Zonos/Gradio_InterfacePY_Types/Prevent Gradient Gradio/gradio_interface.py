...
# Force RTX 3060 (cuda:1), hide 5070Ti
os.environ["CUDA_VISIBLE_DEVICES"] = "1"
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"  # Reduce fragmentation
device = torch.device("cuda:0")  # 3060 is cuda:0
torch.cuda.empty_cache()  # Clear memory at start
    ...
    if speaker_audio is not None and "speaker" not in unconditional_keys:
        if speaker_audio != SPEAKER_AUDIO_PATH:
            ...
            with torch.no_grad():
                ...
                wav = wav.to(device)[:1, :sr*10]  # Limit to 1 channel, 10s
